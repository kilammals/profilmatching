...
echo "<h2>Setting</h2>";
  $set=mysqli_query($koneksi, "SELECT * FROM tbl_variabel");
  $no=0;
...
if ($_GET['aksi']==himpunan) {   
	if ($_GET['di']==simpan) {  
	$id=$_POST['id'];
	mysqli_query($koneksi, " update tbl_himpunan SET  
	...
	if ($_GET['di']==edit) {   
	  $sethf=mysqli_query($koneksi, "SELECT * FROM tbl_himpunan where id='$_GET[ids]'");
...
echo "<h2>Setting</h2>";
  $seth=mysqli_query($koneksi, "SELECT * FROM tbl_himpunan where kelompok='$_GET[id]'");
  $noh=0;
